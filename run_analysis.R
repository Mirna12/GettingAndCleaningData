...
tr_s <- read.table("./UCI HAR Dataset/train/subject_train.txt", header=FALSE, 
                  colClasses="integer", col.names="subject")
#test subjects, 2947x1
ts_s <- read.table("./UCI HAR Dataset/test/subject_test.txt", header=FALSE, 
                  colClasses="integer", col.names="subject")
# 561x2
feature <- read.table("./UCI HAR Dataset/features.txt", header=FALSE, 
                      colClasses="character", col.names=c("feature_no", "feature"))
#
activity_labels <- read.table("./UCI HAR Dataset/activity_labels.txt", header=FALSE, 
                              colClasses=c("integer", "character"), col.names=c("activity_no", "activity"))
#training data from X_train, 7352x561
tr_x <- read.table("./UCI HAR Dataset/train/X_train.txt", header=FALSE, 
                  colClasses="numeric", col.names=feature[[2]])
#test data from X_test, 2947x561
ts_x <- read.table("./UCI HAR Dataset/test/X_test.txt", header=FALSE, 
                  colClasses="numeric", col.names=feature[[2]])
#training label from Y_train, 7352x1
tr_y <- read.table("./UCI HAR Dataset/train/Y_train.txt", header=FALSE, 
                  colClasses="integer", col.names="activity_no")
#test label from Y_test, 2941x1
ts_y <- read.table("./UCI HAR Dataset/test/Y_test.txt", header=FALSE,
                  colClasses="integer", col.names="activity_no")
...
#replace activity numbers in data frame activity with descriptive names instead of numbers
...
#group by subject and activiity, then calculate mean of every subset
